...
class supplierClass:
    def __init__(self,root):
        ...
        #title
        title=Label(self.root,text="Supplier Details",font=("goudy old style",15),bg="#0f4d7d",fg="white").place(x=50,y=10,height=40,width=1000)
...
#================================================================
    ...
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Invoice no. ID already assigned, try different",parent=self.root)
                else:
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("Update suuplier set name=?,contact=?,desc=? where invoice=?",(
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Suppliere Updated Successfully",parent=self.root)
                    self.show()
        except  Exception as ex:
            messagebox.showerror("Error",f"Error due to :{str(ex)}")
    ...
    def delete(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                messagebox.showerror("Error","EInvoice no. must be required",parent=self.root)
            else:
                cur.execute("Select * from supplier where invoice=?",(self.var_sup_invoice.get(),))
                row=cur.fetchone()
                if row==None:
